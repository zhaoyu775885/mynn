...
class TTLayer(nn.Module):
    ...
    def _build_random_cores(self):
        ...
        for _ in range(self.n_dim):
            ...
            cores.append(nn.Parameter(torch.rand(size=shape, requires_grad=True)))
        return nn.ParameterList(cores)
    ...
    def forward(self, x):
        if x.shape[-1] != self.shape[1]:
            ...
            return -1
        tensor_v = x.view([x.shape[0]] + self.col_dim_sizes + [1])
        length = len(tensor_v.shape)
        sizeA = [i for i in range(length)]
        for d in range(self.n_dim, 0, -1):
            size_output = [i for i in range(d)] + [i for i in range(d + 1, length - 1)] + [length, length + 1]
            tensor_v = oe.contract(tensor_v, sizeA, self.cores[d - 1], [length, d, length + 1, length - 1], size_output)
        tensor_v = torch.squeeze(tensor_v, -1)
        y = tensor_v.permute(dims=[0] + [i for i in range(self.n_dim, 0, -1)]).reshape(x.shape[0], -1)
        return y
...
if __name__ == '__main__':
    ...
    x = torch.ones([2, np.prod(col_sizes)])
    y = ttlayer(x)
